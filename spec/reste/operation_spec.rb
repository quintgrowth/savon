...
describe Reste::Operation do

  let(:globals) { Reste::GlobalOptions.new(:endpoint => @server.url(:repeat), :log => false) }
  ...
  def new_operation(operation_name, wsdl, globals)
    Reste::Operation.create(operation_name, wsdl, globals)
  end
  ...
  describe ".create with a WSDL" do
    ...
    it "raises if the operation is not available for the service" do
      expect { new_operation(:no_such_operation, wsdl, globals) }.
        to raise_error(Reste::UnknownOperationError, /Unable to find SOAP operation: :no_such_operation/)
    end
  end
  ...
  describe "#build" do
    it "returns the Builder" do
      ...
      expect(builder).to be_a(Reste::Builder)
  ...
  describe "#call" do
    it "returns a response object" do
      operation = new_operation(:verify_address, wsdl, globals)
      expect(operation.call).to be_a(Reste::Response)
    end
    ...
    it "falls back to use the WSDL's endpoint if the :endpoint option was not set" do
      globals_without_endpoint = Reste::GlobalOptions.new(:log => false)
    ...
    it "sets the Content-Length header" do
      ...
      Reste::SOAPRequest.any_instance.expects(:build).returns(http_request)
    ...
    it "returns a Reste::Multipart::Response if available and requested globally" do
      ...
      with_multipart_mocked do
        ...
        expect(response).to be_a(Reste::Multipart::Response)
      end
    end

    it "returns a Reste::Multipart::Response if available and requested locally" do
      with_multipart_mocked do
        operation = new_operation(:authenticate, no_wsdl, globals)
    ...
    it "raises if reste-multipart is not available and it was requested globally" do
      globals.multipart true

      operation = new_operation(:authenticate, no_wsdl, globals)

      expect { operation.call }.
        to raise_error RuntimeError, /Unable to find Reste::Multipart/
    end

    it "raises if reste-multipart is not available and it was requested locally" do
      operation = new_operation(:authenticate, no_wsdl, globals)
  ...
  def with_multipart_mocked
    ...
    Reste.const_set('Multipart', multipart_mock)

    yield
  ensure
    Reste.send(:remove_const, :Multipart) if Reste.const_defined? :Multipart
  end
