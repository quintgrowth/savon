...
describe Reste::Model do
  ...
  describe ".client" do
    it "returns the memoized client" do
      ...
      expect(model.client).to be_a(Reste::Client)
      expect(model.client).to equal(model.client)
    end

    it "raises if the client was not initialized properly" do
      model = Class.new { extend Reste::Model }

      expect { model.client }.
        to raise_error(Reste::InitializationError, /^Expected the model to be initialized/)
    end
  end
