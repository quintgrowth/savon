...
describe Reste do
  ...
  describe ".observers" do
    ...
    it "allows to register an observer for every request" do
      ...
      Reste.observers << observer

      new_client.call(:authenticate)

      expect(observer.operation_name).to eq(:authenticate)

      expect(observer.builder).to be_a(Reste::Builder)
      expect(observer.globals).to be_a(Reste::GlobalOptions)
      expect(observer.locals).to  be_a(Reste::LocalOptions)
    end
    ...
    it "raises if an observer returns something other than nil or an HTTPI::Response" do
      ...
      expect { new_client.call(:authenticate) }.
        to raise_error(Reste::Error, "Observers need to return an HTTPI::Response " \
